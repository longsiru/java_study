...
//		}
		...
		/*
		 * 8. int타입의 변수 num 이 있을 때, 각 자리의 합을 더한 결과를 출력하는 코드를 완성하라. 만일 변수 int num의 값이
		 * 12345라면, ‘1+2+3+4+5’의 결과인 15를 출력하라. (문자 변환 금지)
		 */
//		int num = 12345;
//		int sum = 0;
//		while (num > 0) {
...
//		System.out.println("sum= " + sum);
		...
		/*
		 * 9. 피보나치(Fibonnaci) 수열(數列)은 앞을 두 수를 더해서 다음 수를 만들어 나가는 수열이다. 예를 들어 앞의 두 수가 1과
		 * 1이라면 그 다음 수는 2가 되고 그 다음 수는 1과 2를 더해서 3이 되어서 1,1,2,3,5,8,13,21,... 과 같은 식으로
		 * 진행된다. 1과 1부터시작하는 피보나치수열의 10번째 수는 무엇인지 계산하는 프로그램을 완성하시오.
		 */
//		int num1 = 1;
//		int num2 = 1;
//		int num3 = 0;
...
//		}
		...
		/*
		 * 11. 다음은 숫자맞추기 게임을 작성한 것이다. 1과 100사이의 값을 반복적으로 입력해서 컴퓨터가 생각한 값을 맞추면 게임이 끝난다.
		 * 사용자가 값을 입력하면, 컴퓨터는 자신이 생각한 값과 비교해서 결과를 알려준다. 사용자가 컴퓨터가 생각한 숫자를 맞추면 게임이 끝나고 몇
		 * 번 만에 숫자를 맞췄는지 알려준다. 네모에 알맞은 코드를 넣어 프로그램을 완성하시오.
		 */
...
//				System.out.println("작은 숫자 입력하세요");
//			}else {
...
//		} while(true);
		...
		/*
		 * 12. while 문과 Scanner의 nextLine ( ) 메소드를 이용해서 다음 실행 결과와 같이 키보드로부터 입력된 데이터로 예금,

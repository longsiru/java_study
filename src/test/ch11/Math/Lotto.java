...
public class Lotto {
	...
	public static void main(String[] args) {
		//선택번호(내가 선택한 번호)
		int[] selectNumber = new int[6]; //lotto범호 6개가 저장될 배열
		Random random = new Random(3);
		System.out.println("선택번호");
		
		  //random number 6개를 생성을 해서 배열에 넣어줌.
		for(int i=0; i<6; i++) {
		...
		//당첨번호(컴퓨터에서 주는 번호)
		int[] winNumber = new int[6]; //lotto범호 6개가 저장될 배열
		...
		Arrays.sort(selectNumber);  //비교하기 전에 배열 값을 정렬: 배열 내부의 값들이 정렬된 상태로 바꿘다.
